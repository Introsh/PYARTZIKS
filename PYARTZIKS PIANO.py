...
class Note:
    def __init__(self, game, target_name, is_special=False):
        ...
        piano_pos = self.game.piano.getPos(self.game.render)
        ...
        # Nouvelle méthode : position mondiale du centre de la touche
        ...
        # ✅ Position de spawn façon Synthesia horizontale (alignée correctement avec la touche)
        spawn_depth = self.game.note_spawn_distance

        spawn_pos = self.target_pos + LVector3(0, spawn_depth, 0)
        self.model.setPos(spawn_pos)
        print(f"🎯 Note générée vers {self.target_name}")
        print(f" - Pos cible (target_pos) : {self.target_pos}")
        print(f" - Pos de spawn : {spawn_pos}")
        self.model.setPos(spawn_pos)
        self.next_target_pos = self.target_pos  # trajectoire droite vers la touche (axe Y uniquement, comme Synthesia)
        self.speed = self.game.note_speed

                # 📍 Position du bloc ajustée pour apparaître devant le piano
        # ❌ Supprimé : ancienne logique de position de spawn devant le piano (remplacée)
    ...
    def hit_target(self):
        print(f"Note {self.target_name} a été touchée !")
        self.model.removeNode()  # Retirer la note du jeu
        if self in self.game.active_notes:
            self.game.active_notes.remove(self)  # Retirer de la liste des notes actives
        self.game.taskMgr.remove(self.task_name)  # Supprimer la tâche associée à cette note
        print(f"Note {self.target_name} supprimée du jeu.")

    def setup_collision(self):
            coll_sphere = CollisionSphere(0, 0, 0, 0.2)
    ...
    def update(self, task):
        ...
        if not target_node.isEmpty():
            min_bound, max_bound = target_node.getTightBounds()
            if min_bound.almostEqual(max_bound):
                    return task.cont

            # Convertir les bornes en coordonnées mondiales
            min_bound_world = target_node.getMat(self.game.render).xformPoint(min_bound)
            max_bound_world = target_node.getMat(self.game.render).xformPoint(max_bound)

            note_pos = self.model.getPos(self.game.render)

            # Collision avec la bounding box de la touche
            tolerance = 0.05  # marge d’erreur pour touches fines
            if (min_bound_world.x - tolerance <= note_pos.x <= max_bound_world.x + tolerance and
                min_bound_world.y - tolerance <= note_pos.y <= max_bound_world.y + tolerance and
                min_bound_world.z - tolerance <= note_pos.z <= max_bound_world.z + tolerance):

                # Vérification de la pression de la barre d'espace
                if self.game.mouseWatcherNode.isButtonDown('space'):
                    print(f"🎶 Note jouée pour {self.target_name} car espace appuyée et collision détectée.")
                    self.game.play_sound_for_key(self.target_name)  # Joue le son de la touche en collision
                    self.hit_target()  # Retirer la note du jeu
                    return task.done  # Note disparue après le hit

        # Déplacement de la note
        direction = LVector3(0, self.target_pos.y - current_pos.y, 0)
        direction.normalize()
        self.model.setPos(current_pos + direction * self.speed)

        # Si la note dépasse un certain seuil (par exemple Y < -10), elle est supprimée comme "ratée"
        if current_pos.y < -10:
            print(f"💥 Note ratée : {self.target_name}")
